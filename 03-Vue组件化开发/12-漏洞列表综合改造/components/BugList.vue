<template>
  <main v-show="lists.length">
    <table>
      <thead>
        <tr>
          <th class="select">全选 <input type="checkbox" v-model="isAll"/></th>
          <th class="content">Bug描述信息</th>
          <th class="operate">操作</th>
        </tr>
      </thead>
      <tbody>
        <BugItem v-for="bug in lists" :key="bug.id" :bug="bug"/>
      </tbody>
    </table>
  </main>
</template>
...
export default {
  name: 'BugList',
  props : ['lists', "isSelectAllCallback"],
  components : {BugItem},
  computed : {
    resolvedCount() {
    ...
    isAll : {
      ...
      set(value) {
        this.$emit('isSelectAllCallback', value);
      }
     }
  }
}
